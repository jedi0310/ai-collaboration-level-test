...
- D1 binding 名称固定为 `DB`。
- D1 表 `submissions` 保存用户画像、答题结果、本地诊断、DeepSeek Markdown 报告和基础访问元数据。
...
- 报告页新增“下载/保存 PDF”按钮，使用 `window.print()` 和 print CSS，不引入依赖。
- 页面增加隐私提示：公开测试版会保存名字/网名、行业、职业、答题结果和生成报告，用于改进测试；提醒不要填写手机号、身份证、详细地址、公司机密等敏感信息。

### 文件

- `app.js`：隐藏访问者 API Key 输入，改为 Worker 代理生成报告，增加公开测试版 payload 和 PDF 按钮。
