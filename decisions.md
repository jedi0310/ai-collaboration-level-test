...
## 2026-06-18 测试阶段临时关闭 DeepSeek 生成限次

### 决策

用户需要多次跑完整测试和报告链路，因此当前测试阶段临时关闭前端“同一浏览器、同一昵称每天 3 次”的 DeepSeek 报告生成拦截。

代码保留限次函数和 `DAILY_REPORT_LIMIT`，但通过 `REPORT_LIMIT_ENABLED = false` 暂停拦截和次数扣减，方便正式发布前恢复或迁移到代理层。

### 影响

- 测试阶段可以反复生成 DeepSeek 个性化报告。
- 页面不再显示“今天剩余次数”或“每天 3 次”文案。
- 正式公开发布前，需要重新确认限流策略。
- 如果使用站长自己的 DeepSeek Key，真实限流必须放到 Cloudflare Worker 或 Vercel Function 代理层，不能依赖前端 localStorage。

## 2026-06-18 DeepSeek 公开版代理方案

### 决策

后续公开传播版不希望最终访问者自己填写 DeepSeek API Key。由于 GitHub Pages 前端无法安全保存站长 Key，下一阶段建议采用：

- GitHub Pages 托管静态前端。
- Cloudflare Worker 或 Vercel Function 作为 DeepSeek API 代理。
- 站长 DeepSeek API Key 只放在代理服务环境变量中。
- 前端只请求代理接口，不接触真实 Key。

### 影响

代理层可以承担真实限流、日志、错误处理和成本保护。当前阶段不实现 Worker 或 Vercel Function，只作为正式发布前的待办和架构边界记录。

## 2026-06-18 公开测试版采用 Cloudflare Worker + D1

### 决策

用户已完成 Cloudflare 注册并创建 D1 数据库 `ai-test-reports`。公开测试版采用：

- GitHub Pages 托管静态前端。
- Cloudflare Worker `ai-test-deepseek-proxy` 代理 DeepSeek。
- DeepSeek API Key 只放在 Cloudflare Worker Secret `DEEPSEEK_API_KEY`。
- 后台访问 token 只放在 Cloudflare Worker Secret `ADMIN_TOKEN`。
- D1 binding 名称固定为 `DB`。
- D1 表 `submissions` 保存用户画像、选填联系方式、答题结果、本地诊断、DeepSeek Markdown 报告和基础访问元数据。

### 影响

- 访问者不再填写 DeepSeek API Key。
- 前端从直接请求 DeepSeek 改为请求 Worker endpoint：
...
- 画像字段规则调整为：名字/网名必填，行业必填，职位选填，联系方式选填；联系方式可填手机号或微信号，不填也能继续测试。
- 报告页“下载 PDF”改为 html2pdf.js 直接生成文件；如果 PDF 生成失败，自动下载 Markdown 报告，不再直接弹出打印界面。
- 页面增加隐私提示：公开测试版会保存名字/网名、行业、选填职位、选填联系方式、答题结果和生成报告，用于改进测试和后续反馈；提醒不要填写身份证、详细地址、公司机密等敏感信息。

### 文件

- `app.js`：隐藏访问者 API Key 输入，改为 Worker 代理生成报告，增加公开测试版 payload、选填联系方式和 PDF 下载按钮。
...
- 曾增加前端软限制：同一浏览器、同一昵称每天最多生成 3 次 DeepSeek 报告；后续按用户要求，测试阶段已临时关闭该拦截，答题测试本身仍不限。
- README、tasks、decisions 和设计计划已同步说明：测试阶段不限次，公开版若使用站长 Key，需 Worker/Vercel 代理层做真实限流。

### 当前文件状态

- `app.js`：包含开始页、用户画像页、8 题答题页、报告页、本地评分、基础报告 fallback、DeepSeek 生成、用户画像 payload、本地 Key 保存；测试阶段暂不限次生成报告。
- `styles.css`：保持 Anthropic 式克制产品风格，支持手机端和桌面端。
- `README.md`、`tasks.md`、`decisions.md`、`docs/plans/2026-06-18-ai-collaboration-level-test-design.md`：已记录当前产品状态、测试阶段限次关闭、代理层限流边界和后续部署方向。
